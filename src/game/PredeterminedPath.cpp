...
PredeterminedPath::PredeterminedPath(const Pos& start, const Pos& end) {
	...
	const double Y_DISTANCE_TO_END = std::abs(end.getY() + end.getHeight() / 2 - Y_START);
	const double DISTANCE_TO_END = std::sqrt(X_DISTANCE_TO_END* X_DISTANCE_TO_END + Y_DISTANCE_TO_END* Y_DISTANCE_TO_END);
	...
	while (!lines.at(lines.size() - 1)->endPos.isCollided(end)) {
		...
		double x_distance_to_end = std::abs(end.getX() + end.getWidth() / 2 - PREVIOUS_LINE_END.getX());
		double y_distance_to_end = std::abs(end.getY() + end.getHeight() / 2 - PREVIOUS_LINE_END.getY());
		//angle from the end of the first line to the end point's middle
		double angle_to_end = std::atan(y_distance_to_end / x_distance_to_end);

		//fill out the other side lengths of the triangle with an initial longer first line
		double triangle_adjacent_length = LINE_LENGTH * std::cos(angle_to_end);
		double triangle_opposite_length = LINE_LENGTH * std::sin(angle_to_end);

		//the distnace from the previous line to the end
		Pos translatedEndPos(end.getX() + end.getWidth() / 2,end.getY() + end.getHeight() / 2);
		double distance = PREVIOUS_LINE_END.getDistanceTo(translatedEndPos);

		//the maximum randomness the line can get and this gets the smaller the closer to the end it gets
		double maxRandomDistance = (distance / DISTANCE_TO_END) * 25;

		//create a random distance using the max above
		double randomiziedLength = Util::random(0,maxRandomDistance);
